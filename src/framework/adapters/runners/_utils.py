...
def find_project_root() -> Path:
    ...
    Walks up from the framework package looking for a ``pyproject.toml``.
    When none is found (e.g. ``uv tool install framework``), falls back
    to ``~/.framework/`` so that benchmark venvs and caches still have a
    stable home directory.
    """
    for parent in Path(__file__).resolve().parents:
        if (parent / "pyproject.toml").exists():
            return parent
    fallback = Path.home() / ".framework"
...
def serialize_kwargs(kwargs: dict[str, Any]) -> tuple[str, str]:
    """Serialize kwargs for the ``framework serve`` CLI.
...
# Framework's own settings travel through runtime.json, not env vars.
#
...
def prepare_subprocess_env() -> dict[str, str]:
    ...
    :data:`_FORWARD_PREFIXES`).  Framework context and settings travel
    via ``runtime.json`` (see :func:`inject_framework_env`), so no
    ``FRAMEWORK_*`` vars need to be forwarded here.
    """
    import os
...
def inject_framework_env(env: dict[str, str], role: Role | None = None) -> None:
    ...
    ``FRAMEWORK_RUNTIME_FILE``.  When *role* is ``None`` the child
    inherits whatever ``FRAMEWORK_RUNTIME_FILE`` is set in the current
    ...
    runtime file via :func:`init_context` — no individual ``FRAMEWORK_*``
    env vars are needed.
    ...
    if role is not None:
        ...
        env["FRAMEWORK_RUNTIME_FILE"] = str(runtime_path)
    else:
