...
class DockerRunner:
    ...
    def _ensure_image(self) -> str:
        ...
        if self._dockerfile:
            tag = f"framework-runner-custom:{hash(self._dockerfile) & 0xFFFFFFFF:08x}"
        ...
        from ...environment.helpers import get_framework_install_target

        project_root, packages = get_framework_install_target()
    ...
    def start(self) -> ObjectProxy:
        ...
        # Framework context and settings travel via runtime.json, not env.
        env = prepare_subprocess_env()
        inject_framework_env(env, role=self._role)
        ...
        # block those writes.
        runtime_file = env.get("FRAMEWORK_RUNTIME_FILE")
        if runtime_file:
