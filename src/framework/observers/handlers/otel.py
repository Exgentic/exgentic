...
class OtelTracingObserver(Observer):
    ...
    def on_run_start(self, run_config) -> None:
        ...
        self._run_attributes = {
            "framework.benchmark.slug_name": bench_entry.slug_name if bench_entry else run_config.benchmark,
            "framework.benchmark.subset": run_config.subset or "",
            "framework.benchmark.agent.name": agent_entry.slug_name if agent_entry else run_config.agent,
            "framework.agent.slug": run_config.agent,
            "framework.run.id": get_run_paths().run_id,
        }
        if model_name:
    ...
    def on_session_enter(self, session_id: str, task_id: str | None) -> None:
        ...
        bench_name = self._run_attributes.get("framework.benchmark.slug_name", "unknown_benchmark")
        subset = self._run_attributes.get("framework.benchmark.subset", "subset")
        ...
        span_manager.set_heritable_attribute("framework.session.id", session_id)
        if task_id is not None:
            span_manager.set_attribute("framework.session.task_id", task_id)
    ...
    def on_session_creation(self, session) -> None:
        ...
        if get_settings().otel_record_content:
            ...
            if task is not None:
                span_manager.set_attribute("framework.session.task", task)

        for action in session.actions:
            prefix = f"framework.session.action.{action.name}"
        ...
        span_manager.set_attribute("framework.session.tools", _serialize_to_json(tools_list))

        for k, v in session.context.items():
            otel_value = to_otel_attribute_value(v)
            if otel_value is not None:
                span_manager.set_attribute(f"framework.context.{k}", otel_value)

    def on_session_start(self, session, agent, observation) -> None:
        self._session_agents[session.session_id] = agent
        span_manager = self._span_managers[session.session_id]

        span_manager.set_attribute("framework.session.agent.id", agent.agent_id)
        agent_path = to_otel_attribute_value(agent.paths.agent_dir)
        if agent_path is not None:
            span_manager.set_attribute("framework.session.agent.path", agent_path)
    ...
    def on_session_success(self, session, score, agent) -> None:
        ...
        span_manager.set_attributes(
            {
                "framework.score.success": score.success,
                "framework.score": score.score,
                "framework.score.is_finished": score.is_finished,
                "framework.session.steps": self._session_step_counters[session.session_id],
                "framework.session.task_id": session.task_id,
            }
        )
        for prefix, data in (
            ("framework.score.metrics", score.session_metrics),
            ("framework.score.metadata", score.session_metadata),
        ):
        ...
        self._set_cost_attr(span_manager, "framework.agent.agent_cost", lambda: agent.get_cost())
        self._set_cost_attr(span_manager, "framework.session.cost", lambda: session.get_cost())
    ...
    def on_session_error(self, session, error) -> None:
        ...
        self._set_cost_attr(span_manager, "framework.session.cost", lambda: session.get_cost())
        span_manager.set_attribute("framework.session.task_id", session.task_id)
