...
"""Basic logging setup for Framework components."""
import logging
import os
import sys
...
def get_logger(
    ...
    *,
    ...
    """Get a logger with standard Framework configuration.
    ...
    - If `log_file_path` is not provided: no console by default unless `console=True` or
      `FRAMEWORK_CONSOLE_LOG` is truthy. Optional default file sink via context run dir.
    ...
    if log_file_path:
        ...
        # Non-file logger
        enable_console_env = os.environ.get("FRAMEWORK_CONSOLE_LOG", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        ctx = try_get_context()
        if ctx is not None:
            lf = Path(ctx.output_dir) / "framework.log"  # global log, not run-scoped
            try:
