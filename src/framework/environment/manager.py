...
class EnvironmentManager:
    ...
    def __init__(self, base_dir: Path | None = None) -> None:
        self.base_dir = base_dir or Path.home() / ".framework"
    ...
    def is_installed(self, name: str, *, env_type: EnvType | None = None) -> bool:
        ...
        **or** when the stored framework version no longer matches the
        running version.
        ...
        if env_type not in marker:
            return False
        if self._framework_version_stale(name, env_type):
            return False
    ...
    def _framework_version_stale(self, name: str, env_type: EnvType) -> bool:
        """Return True if the installed framework version differs from the running one."""
        from .helpers import get_framework_version

        marker = self._read_marker(name)
        stored = marker.get(env_type, {}).get("framework_version")
        current = get_framework_version()
        if stored is None:
            _log.info("Environment %s (%s): no framework version recorded, will rebuild", name, env_type)
            return True
        if stored != current:
            _log.info(
                "Environment %s (%s): framework version changed (%s -> %s), will rebuild",
                name,
                env_type,
                stored,
                current,
