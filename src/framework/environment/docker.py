...
from .helpers import find_package_file, get_framework_version, read_lines
...
class DockerBackend:
    ...
    1. **Base image** (``framework-base:{hash}``) — Python + git + uv + the
    ...
    2. **Bench image** (``{name}:{hash}``) — ``FROM framework-base`` with
    ...
    def install(
        ...
        **kwargs: object,
        ...
        if not force and self._image_exists(image_tag):
            return {"image": image_tag, "framework_version": get_framework_version()}
        self._build_image(
            image_tag,
            req_path,
            setup_path,
    ...
    def _install_with_base(
        ...
        *,
        ...
        if not force and self._image_exists(bench_tag):
            return {"image": bench_tag, "framework_version": get_framework_version()}
        ...
        return {"image": bench_tag, "base_image": base_tag, "framework_version": get_framework_version()}
    ...
    def _base_image_tag(cls, project_root: Path) -> str:
        ...
        h.update(b"\x00")
        return f"framework-base:{h.hexdigest()[:12]}"
    ...
    def _build_bench_image(
        ...
        *,
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="framework-bench-"))
        try:
            ...
            if setup_path is not None:
                ...
                lines.append("RUN FRAMEWORK_DOCKER_BUILD=1 bash /tmp/setup.sh")
    ...
    def _build_image(
        ...
        *,
        ...
        py_version = f"{sys.version_info.major}.{sys.version_info.minor}"
        tmp_dir = Path(tempfile.mkdtemp(prefix="framework-docker-"))
        try:
