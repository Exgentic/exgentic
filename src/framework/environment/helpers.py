...
def get_framework_version() -> str:
    """Return the currently running framework version string.

    Falls back to ``"0.0.0"`` when framework is not installed as a
...
def get_framework_install_target() -> tuple[Path | None, list[str] | None]:
    """Return ``(project_root, packages)`` for installing framework itself.

    If running from a source checkout, returns ``(project_root, None)``
    so backends install from source.  Otherwise returns
    ``(None, ["framework==X.Y.Z"])`` matching the running version.
    ...
    Falls back to an unpinned ``framework`` spec for dev versions that
    don't exist on PyPI.
    """
    from ..adapters.runners._utils import find_project_root

    root = find_project_root()
    if (root / "pyproject.toml").exists() and (root / "src" / "framework").is_dir():
        return root, None
    ...
    try:
        ...
        direct_url_text = distribution("framework").read_text("direct_url.json")
        if direct_url_text:
            url = _json.loads(direct_url_text).get("url", "")
            if url.startswith("file://"):
                source_path = Path(url.removeprefix("file://"))
                if (source_path / "pyproject.toml").exists() and (source_path / "src" / "framework").is_dir():
