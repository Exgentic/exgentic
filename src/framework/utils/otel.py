...
class PerSessionFileExporter(SpanExporter):
    ...
    leakage. ``framework.run.id`` is set as a heritable attribute in
    ...
    def export(self, spans: Sequence) -> SpanExportResult:
        try:
            for span in spans:
                ...
                if attrs.get("framework.run.id") != self._run_id:
                    continue
                sid = attrs.get("framework.session.id")
                if not sid:
                    continue
...
def init_tracing_from_env(
    ...
    resource = Resource.create(
        {
            "service.name": service_name or os.getenv("OTEL_SERVICE_NAME", "framework"),
            "service.version": os.getenv("OTEL_SERVICE_VERSION", "1.0.0"),
            "service.namespace": os.getenv("OTEL_SERVICE_NAMESPACE", "framework"),
