...
"""Container reaper: clean up orphaned framework-owned Docker containers.

Each container spawned by framework is tagged with two labels:

* ``framework.owner_pid`` — creator PID
* ``framework.owner_token`` — per-process UUID4 generated at import time
...
LABEL_OWNER_PID = "framework.owner_pid"
LABEL_OWNER_TOKEN = "framework.owner_token"
...
def reap_orphaned_containers(
    *,
    ...
    another framework process (or an unrelated process at a recycled PID)
    owns them.
    """
    log = logger or _log
    stale: list[str] = []
    ...
    if not stale:
        return 0
    log.warning(
        "Reaping %d orphaned framework container(s) from dead owners.",
        len(stale),
    )
...
def reap_own_containers(
    *,
    ...
    if not own:
        return 0
    log.info("Cleaning up %d framework container(s) owned by PID %d.", len(own), target_pid)
