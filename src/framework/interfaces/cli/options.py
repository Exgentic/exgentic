...
from ...utils.settings import FrameworkSettings, get_settings
...
def _validate_set_keys_for_settings(
    ...
    allowed = set(FrameworkSettings.model_fields.keys())
    for group, path, _ in items:
    ...
    specs = [
        ...
        (
            ("--log-level",),
            {
                ...
                "help": "Override FRAMEWORK_LOG_LEVEL for this run",
            },
        ),
        (("--output-dir",), {"default": DEFAULT_OUTPUT_DIR, "show_default": True}),
        (
            ("--cache-dir",),
            {"help": "Base cache directory (overrides FRAMEWORK_CACHE_DIR)"},
        ),
