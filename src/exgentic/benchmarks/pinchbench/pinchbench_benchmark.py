        ...
        tasks.append(
            {
                ...
                "grading_criteria": _extract_grading_criteria(parsed.get("Grading Criteria", "")),
...
class PinchBenchSession(Session):
    ...
    def __init__(
        ...
        if idx < 0 or idx >= len(self._task_index):
            raise ValueError(f"PinchBench task index {idx} out of range (0..{len(self._task_index) - 1})")
    ...
    def score(self) -> SessionScore:
        ...
        if grading_type in ("automated", "hybrid"):
            ...
            if grading_code:
                ...
                auto_breakdown = _run_automated_grade(grading_code, transcript, workspace)
        ...
        if grading_type in ("llm_judge", "hybrid"):
            ...
            if rubric:
                llm_score, _llm_breakdown, llm_notes = _run_llm_judge(
                    prompt=self._task_data["prompt"],
        ...
        return SessionScore(score=final_score, success=success, is_finished=True)
...
class PinchBenchEvaluator(Evaluator):
    ...
    def list_tasks(self) -> list[str]:
        ...
        if self._category_filter:
            tasks = [t for t in tasks if t.get("category") == self._category_filter]
    ...
    def aggregate_sessions(self, sessions: list[SessionIndex]) -> BenchmarkResults:
        ...
        for paths in self.get_sessions_paths(sessions):
            ...
            try:
                ...
                raise FileNotFoundError(f"Missing benchmark result for session '{paths.session_id}' at {fp}") from err
            except Exception:
