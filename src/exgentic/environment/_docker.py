...
from ._helpers import find_package_file, read_lines
...
def install(
    ...
    *,
    ...
    Args:
        name: Environment name (used in the image tag).
    ...
    image_tag = _image_tag(name, req_path, setup_path, sysdeps_path)

    if not force and _image_exists(image_tag):
        return {"image": image_tag}

    _build_image(image_tag, req_path, setup_path, sysdeps_path)
    return {"image": image_tag}
