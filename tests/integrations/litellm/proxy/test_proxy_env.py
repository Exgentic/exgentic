...
def test_proxy_passes_context_env(monkeypatch):
    ...
    monkeypatch.setattr("framework.integrations.litellm.proxy.subprocess.Popen", _fake_popen)
    monkeypatch.setattr("framework.integrations.litellm.proxy._is_port_open", lambda *_a, **_k: True)
    monkeypatch.setattr("framework.integrations.litellm.proxy._is_proxy_ready", lambda *_a, **_k: True)
    ...
    with LitellmProxy(model="openai/gpt-4o-mini", port=49998, startup_timeout=1.0):
        env = captured["env"]
        assert env["FRAMEWORK_RUNTIME_FILE"] == "/tmp/out/run-proxy/sessions/sess-1/benchmark/runtime.json"
