...
push_to_ghcr() {
    ...
    local runtime=$2
    local tag=$3
    local local_image="localhost/exgentic-a2a-${agent}:${tag}"
    local ghcr_image="ghcr.io/exgentic/exgentic-a2a-${agent}:${tag}"
    
    print_step "Pushing ${agent} to GitHub Container Registry..."
...
build_agent() {
    ...
    if $build_cmd \
        ...
        if [ "$should_push" = "true" ]; then
            echo ""
            if push_to_ghcr "$agent" "$runtime" "$tag"; then
                return 0
            else
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                cat << EOF
Usage: $0 AGENT_NAME [--tag TAG] [--use-cache] [--push]
...
Examples:
  $0 tool_calling                           # Build without cache (default)
  ...
  $0 tool_calling --tag v1.0.0 --use-cache --push  # Build v1.0.0 with cache and push
...
EOF
    ...
    if [ -z "$AGENT" ]; then
        ...
        echo ""
        echo "Usage: $0 AGENT_NAME [--tag TAG] [--use-cache] [--push]"
        echo ""
        echo "Example: $0 tool_calling --tag v1.0.0 --push"
    ...
    for agent in "${AGENTS[@]}"; do
        if build_agent "$agent" "$RUNTIME" "$TAG" "$USE_CACHE" "$PUSH_TO_GHCR"; then
            ((SUCCESS_COUNT++))
            if [ "$PUSH_TO_GHCR" = "true" ]; then
                ((PUSH_SUCCESS_COUNT++))
            fi
    ...
    if [ $FAIL_COUNT -gt 0 ]; then
        ...
        if [ "$PUSH_TO_GHCR" = "true" ]; then
            ...
            for agent in "${AGENTS[@]}"; do
                echo "  https://github.com/orgs/Exgentic/packages/container/package/exgentic-a2a-${agent}"
            done
        fi
    fi
}
